...
void updatetop2(uint32_t ind, size_t count, _statStruct& out_f) { // for forward strand
...
void updatetop2(size_t count_f, uint32_t ind, size_t count, statStruct& out_r) { // for reverse strand
...
template <typename T>
void _countHit(kmerCount_umap& kmers1, kmerCount_umap& kmers2, T& kmerDBi, size_t nloci, statStruct& out) {
...
// used when no baitDB
template <typename T>
size_t countHit(kmerCount_umap& kmers1, kmerCount_umap& kmers2, T& kmerDBi, size_t nloci, uint16_t Cthreshold, float Rthreshold = 0.5) {
    statStruct stat;
...
template <typename T>
size_t countHit(kmerCount_umap& kmers1, kmerCount_umap& kmers2, T& kmerDBi, size_t nloci, uint16_t Cthreshold, float Rthreshold, vector<uint16_t>& contamination) {
    statStruct stat;
    _countHit(kmers1, kmers2, kmerDBi, nloci, stat);
...
size_t countHit(vector<size_t>& kmers1, vector<size_t>& kmers2, vector<size_t>& dup1, vector<size_t>& dup2, 
                T& kmerDBi, size_t nloci, uint16_t Cthreshold, float Rthreshold = 0.5) {
    statStruct stat;
    _countHit(kmers1, kmers2, dup1, dup2, kmerDBi, nloci, stat);
...
void countDupRemove(vector<size_t>& kmers, vector<size_t>& dup) {
    std::sort(kmers.begin(), kmers.end());
    size_t endpos = *(std::lower_bound(kmers.begin(), kmers.end(), -1)); // index of the first occurrence of invalid kmer

    size_t last = kmers[0], dupCount = 1, uniqCount = 0;
    for (size_t i = 1; i < endpos; i++) {
        if (last == kmers[i]) { dupCount++; }
        else { 
            dup.push_back(dupCount);
            kmers[uniqCount] = last;
            last = kmers[i];
            dupCount = 1;
            uniqCount++;
        }
    ...
    dup.push_back(count);
    kmers[uniqCount] = last;
    kmers.resize(uniqCount+1);
    assert(dup.size() == kmers.size());
}
...
void CountWords(void *data) {
    kmerDBi_mph_class& kmerDBi = *((Counts*)data)->kmerDBi;
    ...
    while (true) {
        ...
        if (interleaved) { // TODO: test balanced cth
            ...
            while (seqi < seqs.size()) {
                ...
                vector<size_t> kmers1, kmers2, dup1, dup2;
                read2kmers(kmers1, seq, k); // stores numeric kmers
                read2kmers(kmers2, seq, k);
                kmerDBi.map(kmers1); // stores hashed values
                kmerDBi.map(kmers2);
                countDupRemove(kmers1, dup1); // stores unique sorted hash vals in kmers; count of occ in dup
                countDupRemove(kmers2, dup2);

                size_t ind;
                if (bait) {
                    ind = countHit(kmers1, kmers2, kmerDBi, nloci, Cthreshold, Rthreshold, contamination);
                } else {
                    ind = countHit(kmers1, kmers2, dup1, dup2, kmerDBi, nloci, Cthreshold, Rthreshold);
                }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        cerr << endl;
        cerr << "Usage: nuQueryFasta [-b] [-e] [-t] -k -key -mph <-q | -qs> <-fq | -fqi | fai> -o -p -cth -rth" << endl;
        cerr << "Options:" << endl;
        ...
        cerr << "  -key   The file name of the sorted unique kmers in kmerDBi" << endl;
        cerr << "  -mph   The file name of the minimal perfect hash function used from kmerDBi" << endl;
    ...
    vector<string>::iterator it_key = find(args.begin(), args.begin()+argc, "-key");
    vector<string>::iterator it_mph = find(args.begin(), args.begin()+argc, "-mph");
    ...
    size_t ind_f = std::min( {ind_fq, ind_fqi, ind_fai, ind_fb} ) + 1;
    string trFname;
    string keysFname = *(it_key+1);
    string mphFname = *(it_mph+1);

    ifstream mphFile(mphFname);
    ifstream keysFile(keysFname);
    assert(mphFile);
    assert(keysFile);
    ...
    // new implementation, read in kmers using mph and kmerDBi_mph_class
    kmerDBi_mph_class kmerDBi(keysFname, mphFname);
