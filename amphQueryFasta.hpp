#ifndef A_MPH_QUERYFASTA_HPP_
#define A_MPH_QUERYFASTA_HPP_

//#include "emphf_test/common.hpp"
//#include "emphf_test/perfutils.hpp"
//#include "emphf_test/base_hash.hpp"
//#include "emphf_run/compute_mphf_generic.hpp"
//#include "emphf_run/hypergraph.hpp"
//#include "emphf_run/hypergraph_sorter_seq.hpp"
#include "KmerIO.hpp"
#include "emphf_run/test_mphf_generic_numeric.hpp"
#include "emphf_run/mphf_numeric.hpp"
#include "emphf_run/base_hash_numeric.hpp"
...
typedef unordered_map<size_t, std::atomic_size_t> kmer_aCount_umap;
...
size_t getNextKmer(size_t& kmer, size_t beg, string& read, size_t k){
    ...
    while (validlen != k){
        ...
        if (std::find(alphabet, alphabet+4, read[beg + validlen]) == alphabet+4){
            beg = beg + validlen + 1;
            validlen = 0;
        } else {
            validlen += 1;
...
std::tuple<adj_dict, size_t> buildAdjDict(kmerCount_umap& kmers, size_t k) {
    adj_dict adj;
    string s, t;
    size_t max = 0, m;
    for (auto& p : kmers){
        s = decodeNumericSeq(p.first >> 2, k-1);
        t = decodeNumericSeq(p.first % (1ULL << 2*(k-1)), k-1);
        m = kmers[p.first];
        adj[s][t] += m;
        if (m > max){
            max = m;
    ...
    return std::make_tuple(adj, max);
}

void writeDot(string outfpref, size_t i, adj_dict &adj) {
    std::ofstream fout;
    fout.open(outfpref + ".loci." + std::to_string(i) + ".dot");
    assert(fout.is_open());
    fout << "strict digraph \"\" {" << '\n';
    for (auto& p : adj){
        for (auto& q : p.second){
...
void writeDot(string outfpref, int i, adjAttr_dict &adjAttr) { // function polymorphism: adj with attributes information
    ...
    if (i == -1) {
        ...
        fout.open(outfpref + "." + std::to_string(i) + ".unique.dot");
    }
    assert(fout.is_open());
    fout << "strict digraph \"\" {" << '\n';
    for (auto& p : adjAttr){
        for (auto& q : p.second){
            ...
            fout << ", Label = " << std::to_string(q.second[1]) << "];" << '\n';
        }
    }
    fout << "}";
    fout.close();
    ...
    void updatesets(string *s, string *t, int mode, int oldlabel = -1, int newlabel = -1) {
        ...
        if (mode == 2) {        // new set is created
            ...
            nodes[newlabel].insert(nodes[newlabel].end(), 
                std::make_move_iterator(nodes[oldlabel].begin()),
                std::make_move_iterator(nodes[oldlabel].end()));
            ...
            nodes[newlabel].insert(nodes[newlabel].end(), 
                std::make_move_iterator(tmpnodes.begin()),
                std::make_move_iterator(tmpnodes.end()));       
            nodes[oldlabel].clear();
            nset--;
...
struct kipair_s {
    size_t k;
    unordered_set<uint32_t>* i = NULL;
};

typedef vector<kvpair_s> kmerDB_t;
typedef vector<kipair_s> kmerDBi_t;

class kmerDBi_mph_class {
public:
    kmerDBi_mph_class(string& keysFname, string& mphFname) {

        emphf::test_mphf_main<emphf::mphf<emphf::jenkins64_hasher>>(keysFname, mphFname, mphfn, true); // run correctness check
        //compute_mphf_main<hypergraph_sorter_seq<hypergraph<uint32_t>>,
        //                  hypergraph_sorter_seq<hypergraph<uint64_t>>,
        //                  jenkins64_hasher>(keysFname, mphFname, mphfn);
        DBsize = mphfn.size();
        kmerDBi.resize(DBsize);
        isempty.resize(DBsize, true);
    }

    void map(vector<size_t>& kmers) { // set the hash val of keys not in DB to largest val (-1)
        for (size_t i = 0; i < kmers.size(); i++) {
            size_t ind = mphfn.lookup(kmers[i]);
            if (isempty[ind]) { kmers[i] = -1; }
            else {
                if (kmerDBi[ind].k != key) { kmers[i] = -1; }
                else { kmers[i] = ind; }
            }
        }
    }

    bool count(size_t key) {
        size_t ind = mphfn.lookup(key);
        if (isempty[ind]) { return false; }
        else {
            if (kmerDBi[ind].k == key) { return true; }
            else { return false; }
        }
    }

    // will cause error if key is not in kmerDBi
    // use this->count() before calling this method
    unordered_set<uint32_t>& operator [](size_t key) { 
        size_t ind = mphfn.lookup(key);
        //assert(count(key)); // only for testing. should be removed later on.
        if (isempty[ind]) {
            kmerDBi[ind].k = key;
            kmerDBi[ind].i = new unordered_set<uint32_t>;
            isempty[ind] = false;
        }
        return *(kmerDBi[ind].i);
    }
...
private:
    kmerDBi_t kmerDBi;
    vector<bool> isempty;
    emphf::mphf<emphf::jenkins64_hasher> mphfn;
    size_t DBsize;

    void insert(size_t key, uint32_t locus) {
        size_t ind = mphfn.lookup(key);
        if (isempty[ind]) {
            kmerDBi[ind].i = new unordered_set<uint32_t>;
            isempty[ind] = false;
