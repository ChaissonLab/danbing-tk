...
void CountWords(void *data) {
	...
	uint64_t nReads_ = 0, nShort_ = 0, nThreadingReads_ = 0, nFeasibleReads_ = 0, nAsgnReads_ = 0, nSubFiltered_ = 0, nKmerFiltered_ = 0, nQualFiltered_ = 0, nBaitFiltered_ = 0, nLocusAssignFiltered_ = 0, nQCFiltered_ = 0;
	...
	uint64_t& nQCFiltered = *((Counts*)data)->nQCFiltered;
	...
	vector<uint8_t>& qcFilter = *((Counts*)data)->qcFilter;
...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		cerr << '\n'
		     << "Usage: danbing-tk [-bu] [-ka] [-qc] [-k] [-kf] [-cth] [-qth] [-b] [-c] [-r] [-p] <-fa|-fq> -qs <-o|-on>\n"
		     << "Options:\n"
			 << "-Input:\n"
	...
	bool bait = false, aug = false, threading = false, correction = true, tc = false, aln = false, aln_minimal=false, okam = true, g2pan = false, skip1 = false, writeKmerName = false, outputBubbles = false, invkmer = false, isFastq = false, trackBait = false, qc = false;
	...
	string trPrefix, trFname, fastxFname, outPrefix, qcFn;
	string baitFname = "";
	...
	uint64_t nReads = 0, nThreadingReads = 0, nFeasibleReads = 0, nAsgnReads = 0, nSubFiltered = 0, nKmerFiltered = 0, nBaitFiltered = 0, nQualFiltered = 0, nLocusAssignFiltered = 0, nQCFiltered = 0;
	for (uint64_t i = 0; i < nproc; ++i) {
		...
		counts.nQCFiltered = &nQCFiltered;
	...
	cerr << nReads << " reads processed in total.\n"
		 ...
		 << nQCFiltered << " reads removed by QC filter.\n"
