...
1. building repeat-pan genome graphs (RPGG) for a set of "genotypable" VNTRs given haplotype-resolved assemblies (`danbing-tk build`),
...
### Download Releases
The lastest version of RPGG required for genotyping is distributed along with each release.

To run `danbing-tk build` `danbing-tk predict` or leave-one-out analysis on the given RPGG, please refer to the original paper for accession numbers of the assemblies and SRS datasets used to build the RPGG.
...
### danbing-tk build
Required inputs: haplotype-resolved assemblies, matching SRS data, reference genome (major chromosomes only without minor contigs) and bed file of tandem repeat regions (optional)
...
Submitting jobs to cluster is preferred as `danbing-tk build` is compute-intensive, ~80 cpu hours for the original dataset. Otherwise, remove `--cluster` and its parameters to run jobs locally.
